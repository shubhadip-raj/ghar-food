  ...
  const ChefCard = ({ chef, showActions }) => (
    <div className="bg-white rounded-2xl border border-amber-100 p-4">
      <div className="flex gap-3">
        <img src={chef.photo_url || 'https://placehold.co/56x56/f97316/white?text=🏠'}
          alt={chef.name} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-semibold text-stone-800">{chef.name}</h3>
            <span className={`text-xs px-2 py-0.5 rounded-full ${chef.status === 'approved' ? 'bg-green-100 text-green-700' : chef.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'}`}>
              {chef.status}
            </span>
          </div>
          <p className="text-sm text-stone-500">{chef.email} · {chef.phone}</p>
          <p className="text-xs text-stone-400">{chef.address}</p>
          <p className="text-xs text-stone-400 mt-1">Specialties: {chef.recipe_list}</p>
          {chef.place_of_origin && <p className="text-xs text-stone-400">From: {chef.place_of_origin}</p>}
        </div>
        {chef.kitchen_photo_url && (
          <img src={chef.kitchen_photo_url} alt="Kitchen" className="w-14 h-14 rounded-xl object-cover hidden sm:block" />
        )}
      </div>
      {showActions && (
              ...
              : orders.map((order) => (
                <div key={order.id} className="bg-white rounded-2xl border border-amber-100 p-4">
                  <div className="flex items-start justify-between flex-wrap gap-2">
                    <div>
                      <p className="font-semibold text-stone-800">#{order.id.slice(0, 8).toUpperCase()} — {order.customer_name}</p>
                      <p className="text-sm text-stone-500">{order.customer_email} · {order.customer_phone}</p>
                      <p className="text-xs text-stone-400">Chef: {order.chefs?.name} | Dish: {order.menus?.name} ({order.menus?.meal_type})</p>
                    </div>
        ...
        {tab === 'settings' && (
          <form onSubmit={saveSettings} className="bg-white rounded-2xl border border-amber-100 p-6 space-y-5">
            ...
            <div className="bg-amber-50 rounded-xl p-4 text-sm text-amber-800">
              ...
              <code className="text-xs block bg-amber-100 p-2 rounded-lg whitespace-pre-wrap">
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="flex gap-3 mt-4">
              <button onClick={() => setRejectModal(null)}
                className="flex-1 border border-stone-200 text-stone-600 py-2.5 rounded-xl font-semibold hover:bg-stone-50 transition">Cancel</button>
              <button onClick={rejectChef}
                className="flex-1 bg-red-500 text-white py-2.5 rounded-xl font-semibold hover:bg-red-600 transition">Reject</button>
            </div>
          </div>
        </div>
      )}
