...
export default function ChefDashboard() {
  ...
  async function postMenu(type) {
    ...
    try {
      ...
      type === 'lunch' ? setLunchForm({ name:'', price:'', description:'', photo:null, preview:'' })
                       : setDinnerForm({ name:'', price:'', description:'', photo:null, preview:'' });
      fetchAll();
    } catch (err) {
      setMsg(p => ({ ...p, [type]: '❌ ' + err.message }));
    } finally {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background:'#FDF6E3' }}>
      <div className="text-center">
        ...
        <p className="font-display text-spice-600">Loading your kitchen…</p>
      </div>
    </div>
  );
  ...
  // Reusable menu card for lunch/dinner
        ...
        {menu ? (
          ...
          <div className="space-y-3">
            <div className="flex gap-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
              {menu.photo_url && (
                <img src={menu.photo_url} alt={menu.name} className="w-16 h-16 rounded-xl object-cover flex-shrink-0" />
              )}
            ...
            <button onClick={() => toggleMenu(menu.id, menu.is_available)}
              className={`w-full text-sm py-2.5 rounded-xl font-semibold transition border ${
                menu.is_available
          ...
          <div className="space-y-3">
            {!canPost && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-700">
                ⏰ You can post this menu during: <strong>{window}</strong>
              </div>
            )}
            <input className={inputCls} placeholder={`Today's ${type} dish *`}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className={`flex items-center gap-2 ${inputCls} cursor-pointer`}>
                ...
                <span className="text-stone-400 text-xs">{form.preview ? 'Change photo' : 'Food photo'}</span>
            ...
            <textarea className={inputCls} rows={2} placeholder="Short description (optional)"
            ...
            <button onClick={() => postMenu(type)} disabled={saving[type] || !canPost}
              title={!canPost ? `Posting window: ${window}` : undefined}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              {chef.photo_url
                ...
                : <div className="w-8 h-8 rounded-full bg-spice-100 flex items-center justify-center text-sm">🧑‍🍳</div>
              }
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                ...
                chef.status === 'rejected' ? 'bg-red-100 text-red-600' :
                'bg-amber-100 text-amber-700'}`}>
                {chef.status === 'approved' ? '✅ Live' : chef.status === 'rejected' ? '✗ Rejected' : '⏳ Pending'}
              </span>
            </div>
            <button onClick={logout} className="text-sm text-stone-400 hover:text-stone-600 transition">Logout</button>
          </div>
        )}
      </header>
        ...
        {tab === 'orders' && (
          <div className="space-y-3">
            {todayOrders.length === 0 ? (
              ...
              <div key={order.id} className="bg-white rounded-2xl border border-amber-100 p-4 flex items-center justify-between gap-3">
                <div>
            ...
            {orders.length > todayOrders.length && (
              <details className="bg-white rounded-2xl border border-amber-100 p-4">
                <summary className="cursor-pointer text-sm text-stone-500 font-semibold">Show all past orders ({orders.length - todayOrders.length} more)</summary>
                <div className="mt-3 space-y-2">
                  {orders.filter(o => o.created_at?.slice(0,10) !== today).map(order => (
                    <div key={order.id} className="flex justify-between text-sm p-2 border-t border-stone-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              {[
                ...
                <div key={k} className="p-3 bg-stone-50 rounded-xl">
                  ...
                  <p className="text-stone-700 mt-0.5 font-medium capitalize">{v}</p>
                </div>
              ))}
